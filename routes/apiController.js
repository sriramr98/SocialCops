...
const loginUserController = async (req, res) => {
    try {
        ...
        if (existingUser) {
            ...
            // since we accept any password,
            // I'm not really checking if passwords match
...
const jsonPatchController = async (req, res) => {
    try {
        ...
        const modifiedJson = jsonPatcher.applyPatch(jsonData, jsonPatch)
            .newDocument;
        ...
        return res
            .status(400)
            .json(
                failureResponse(errorcodes.ERROR_SERVER_ERROR, 'Error applying patch')
            );
    }
};
